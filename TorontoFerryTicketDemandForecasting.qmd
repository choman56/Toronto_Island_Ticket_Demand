...
author: "Clarke A. Homan"
format: html
editor: visual
params:
  github: choman56
  author: "Clarke A. Homan"
  email: "cahoman@gmail.com"
---
...
These are the goals of my pursuing this project:

-   Using R and RStudio, replicate the findings of HeTianning's white paper analyzing the use of SARIMA and Prophet time series models to forecast 365 days into the future based upon historical ferry ridership data obtained from the Toronto ferry site.

-   Compare the effectiveness of using SARIMA versus Prophet in developing models of the ridership time series data.

-   Practice data manipulation and plotting capabilities of R and it's libraries

-   Explore the use of Claude Sonnet in supporting my development of the R code. Although I interacted with Claude as support in reviewing my code and in some cases, correcting and improving it, I did not just ask Claude to write the code for me. I steered the ship.
...
toronto_ferry_data_df |> ggplot(aes(x = Date, y = Counts)) +
  ...
  geom_hline(yintercept = mean(toronto_ferry_data_df$Counts),
...
```{r}
toronto_ferry_data_df$Counts_log <- log(toronto_ferry_data_df$Counts)
...
toronto_ferry_data_df |> ggplot(aes(x = Date, y = Counts_log)) +
  geom_rect(data = highlight_periods, aes(xmin = xmin, xmax = xmax,
                                          ...
                                          alpha = 0.2, inherit.aes = FALSE) +
  geom_line() +           # Add timeseries line
  geom_hline(yintercept = mean(toronto_ferry_data_df$Counts_log),
             linetype = "dashed", color = "red") +
  xlab("Timestamp") + ylab("Redemption Counts") +
...
Now let's plot the first differencing of the log-transformed data to mitigate the inherent seasonality of ferry ridership.
...
pacf_df |> ggplot(aes(x = lag, y = pacf)) +
  geom_ribbon(aes(ymin = -confidence_interval, ymax = confidence_interval), 
              fill = "lightblue", alpha = 0.5) +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = mean(pacf_df$pacf),
             linetype = "dashed", color = "red") +
  geom_segment(aes(xend = lag, yend = 0), linewidth = 1, color = "steelblue") +
  geom_point(size = 1, color = "steelblue") +
...
Now that we have stationary time series data to model, we can now use the seasonally autoregressive integrated moving average (SARIMA) model to help us predict future ticket redemption volumes based historical redemption counts obtained from the website.

The SARIMA model can handle seasonality influences, while it models both seasonal and non-seasonal trends. This model is widely used in a number of fields including economics and finance.

To access the SARIMA modelling package for R, I need to require the "forecast" library, which I obtain from CRAN repository.
...
toronto_ferry_ts <- ts(toronto_ferry_data_df$Counts_log, 
                       frequency = 7)           # 7 for weekly seasonality

sarima_model <- auto.arima(toronto_ferry_ts,
                           seasonal = TRUE,
                           stepwise = FALSE,
                           approximation=FALSE)

sarima_residuals <- checkresiduals(sarima_model)
```

Looking at the residuals from the SARIMA model, we see most of the residuals are within +/- 2.5 standard deviation from mean (0), yet there are numerous "spikes" in the data that are significantly "different" than the 2 standard deviations from the mean. These deviations could be the result of ferry closures, special events, holidays or weather related events. The Ljung-Box test has a very low p-value (0.0002813), which we would reject the null hypothesis that residuals are white noise. There's remaining autocorrelation the model isn't capturing.

Now that we have built a model, let's try predicting the next 12 months using this model.

```{r}
forecasts <- forecast(sarima_model, h = 365)  # Forecast 1 year ahead

# Step 2: Back-transform from log to original scale
forecasts_original <- forecasts
forecasts_original$mean <- exp(forecasts$mean)
forecasts_original$lower <- exp(forecasts$lower)
forecasts_original$upper <- exp(forecasts$upper)
forecasts_original$x <- exp(forecasts$x)

# Step 3: Create a dataframe with actual dates
# Get the last date from your original data
last_date <- max(toronto_ferry_data_df$Date)  # Or whatever your date column is called

# Create future dates
future_dates <- seq(last_date + 1, by = "day", length.out = 365)

# Create historical dates (assuming your data starts from a known date)
# Adjust this based on your actual start date
start_date <- min(toronto_ferry_data_df$Date)
historical_dates <- seq(start_date, by = "day",
                        length.out = length(forecasts_original$x))

# Step 4: Create a custom ggplot
# Prepare data for plotting
forecast_df <- data.frame(
  date = future_dates,
  forecast = exp(forecasts$mean),
  lower80 = exp(forecasts$lower[,1]),
  upper80 = exp(forecasts$upper[,1]),
  lower95 = exp(forecasts$lower[,2]),
  upper95 = exp(forecasts$upper[,2])
)

historical_df <- data.frame(
  date = toronto_ferry_data_df$Date,
  actual = toronto_ferry_data_df$Counts)


# Plot
ggplot() +
  geom_line(data = historical_df, aes(x = date, y = actual), color = "black") +
  geom_line(data = forecast_df, aes(x = date, y = forecast), color = "blue", size = 1) +
  labs(title = "Forecast of Toronto Ferry Ticket Redemptions for Next Year",
       x = "Date",
...
The forecast plot generated using SARIMA exposes a limitation using this model. The Toronto ferry data has multiple cyclicalities. Ridership is affected by day of week where weekends and holidays experience increased ridership, but also ridership is affected by seasonality, more ridership during fair weather periods of the year like summer versus dead of winter.

Unfortunately the SARIMA model is limited in handling multiple cyclicalities. SARIMA can only account for one seasonal pattern. The data has two:

-   Weekly (period=7): Weekday vs weekend ridership

-   Annual (period =365): Summer peaks vs winter lows

When we used the SARIMA model to generate predictions, we used the forecast(sarima_model, h = 7), which accounted for the weekly period, leaving the model to mis-forecast due to annual cyclicalities.

## Using Prophet Modeling to Forecast Ticket Redemptions

Prophet is an open source modeling software from Meta Corporation, that is used to model time series data to help with forecasting future trends. It is based upon a de-composable additive model where data with non-linear trends fit with seasonality trends. This information was summarized from the web article [Time Series Analysis using Facebook Prophet](https://www.geeksforgeeks.org/data-science/time-series-analysis-using-facebook-prophet/)

The steps to using Prophet are the following:

1.  Install and reference the Prophet library (if not already installed) from CRAN

2.  Create a suitable data.frame (the Prophet modeling software requires the data to be passed to it within a specific data.frame structure. The data-frame structure is

    -   column named "ds" which is are the dates of each time series data item in "Date" format
    -   column named "y" which is numeric time series data

3.  Generate the model using the prophet function within the Prophet library

4.  Create future dataframe using the "make_future_dataframe()" function for the period desired

5.  Generate the future forecasted predictions using the generated model created by the prophet function, supplying the future dataframe
...
# Step 2: Prepare your data for Prophet
# Prophet requires a dataframe with columns 'ds' (date) and 'y' (value)
toronto_ferry_data_prophet_df <- data.frame(ds = toronto_ferry_data_df$Date,
                                            y = as.numeric(toronto_ferry_data_df$Counts))

# Step 3: Fit the Prophet model
toronto_ferry_prophet_model <- prophet(toronto_ferry_data_prophet_df,
                                       seasonality.mode = 'additive',  # or 'multiplicative'
                                       yearly.seasonality = TRUE,
                                       weekly.seasonality = TRUE,
                                       daily.seasonality = FALSE)  # usually not needed

# Step 4: Create future dataframe for 1 year (365 days)
toronto_future_prophet <- make_future_dataframe(toronto_ferry_prophet_model,
                                                periods = 365)

# Step 5: Generate the predictions
toronto_forecast <- predict(toronto_ferry_prophet_model,
                            toronto_future_prophet)

# Step 6: # Visualize the forecast
ggplot(toronto_forecast, aes(x = ds)) +
  geom_ribbon(aes(ymin = yhat_lower, ymax = yhat_upper,
                  fill = "80% Interval"), alpha = 0.3) +
  geom_line(aes(y = yhat, color = "Forecast"), linewidth = 1) +
  geom_point(data = toronto_ferry_data_prophet_df,
             aes(y = y, color = "Actual"), size = 0.5) +
  scale_color_manual(values = c("Actual" = "black", "Forecast" = "blue")) +
  scale_fill_manual(values = c("80% Interval" = "cyan")) +
  labs(title = "One Year Toronto Island Ferry Ridership Forecast",
       x = "Year", 
       y = "Redemption Count",
       color = NULL,
       fill = NULL) +
...
The plot above shows ferry ridership between 2015 and 2026, with actual and forecast amounts. The current predictions are well within the 80% confidence band. The future predictions are shown to follow a similar pattern.

## Effectiveness of Prophet model

Unlike SARIMA, which can only model a single seasonal pattern, Prophet is designed to handle multiple seasonalities simultaneously. The Toronto ferry ridership data exhibits both weekly patterns (weekday vs. weekend usage) and annual patterns (summer peaks vs. winter lows) - a characteristic common in many real-world time series datasets. This makes Prophet the more appropriate modeling choice for this analysis.

## Cross Validating Prophet Modeling

Cross validation has several benefits when modeling time series data:

-   Improved performance estimates
-   Improved use of limited data
-   Model over-fitting detection
-   Tests forecast horizon performance

```{r}
# Cross-validation with rolling windows
# initial: minimum training period (e.g., 730 days = 2 years)
# period: spacing between cutoff dates (e.g., 90 days)
# horizon: forecast horizon to evaluate (e.g., 365 days)
toronto_df_cv <- prophet::cross_validation(toronto_ferry_prophet_model,
                                  initial = 730,  # Use first 2 years for initial training
                                  period = 90,    # Move forward 90 days each time
                                  horizon = 365,  # Forecast 1 year ahead
                                  units = 'days')

# Calculate performance metrics
df_p <- prophet::performance_metrics(toronto_df_cv)
head(df_p)
```

Plot the cross validation performance metrics, focusing on performance variables that can handle 0's in the Y data. This excludes mape which when encountering 0 causes a divide by zero (Inf) data ourcome, which the builtin plotting function plot_cross_validation_metric() doesn't handle well.

```{r}
# prophet::plot_cross_validation_metric() has a plotting bug using ggplot2 version 4.0.0
# prophet::plot_cross_validation_metric(toronto_df_cv, metric = 'coverage')

# So let's manually plot the cross-validation model performance coverage
require(gridExtra)

plot_cv_coverage <- ggplot(df_p, aes(x = horizon, y = coverage)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_hline(yintercept = 0.8, linetype = "dashed", color = "red") +
  annotate("text", x=300, y=0.82, label="Ideal 80% coverage", size=2, color = "red") +
  ylim(0.5, 1) +
  labs(title = "Coverage Over Forecast Horizon",
       x = "Horizon (days)",
       y = "Coverage") +
  theme_minimal()

plot_cv_rmse <- ggplot(df_p, aes(x = horizon, y = rmse)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_hline(yintercept = mean(df_p$rmse), linetype = "dashed", color = "red") +
  annotate("text", x=75, y=mean(df_p$rmse)+50,
           label="Average RMSE", size=2, color = "red") +
  labs(title = "RMSE Over Forecast Horizon",
       x = "Horizon (days)",
       y = "RMSE") +
  theme_minimal()

plot_cv_mae <- ggplot(df_p, aes(x = horizon, y = mae)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_hline(yintercept = mean(df_p$mae), linetype = "dashed", color = "red") +
  annotate("text", x=75, y=mean(df_p$mae)+50,
           label="Average MAE", size=2, color = "red") +
  labs(title = "MAE Over Forecast Horizon",
       x = "Horizon (days)",
       y = "MAE") +
  theme_minimal()

grid.arrange(plot_cv_coverage, plot_cv_rmse, plot_cv_mae, nrow = 2, ncol = 2)
```

### Cross Validation Coverage Plot

The coverage plot reveals how well the model's prediction intervals capture actual values across different forecast horizons:

-   **Short-term (37-100 days):** Coverage remains near 80%, very close to the ideal target. This indicates the model's uncertainty estimates are well-calibrated for near-term forecasts, with approximately 80% of actual values falling within the 80% prediction intervals.

-   **Mid-term (150-200 days):** Coverage drops to 68-72%, suggesting the prediction intervals are slightly too narrow. More than 20% of actual values fall outside the predicted ranges, indicating the model underestimates uncertainty at this horizon.

-   **Long-term (250-365 days):** Coverage stabilizes around 70-73%. While below the ideal 80% target, this remains reasonable for year-ahead daily forecasts, though it indicates continued slight underestimation of forecast uncertainty.Cross Validation RMSE Plot

### Cross Validation RMSE Plot

The RMSE plot shows forecast error increases with forecast horizon. Short-term forecasts (37-100 days) have an RMSE around 3,300 passengers, while long-term forecasts (300+ days) have an RMSE of 4,300-4,500 passengers.

This increase is expected: uncertainty compounds as the model forecasts further into the future. Each additional day adds more potential error, causing RMSE to rise steadily before leveling off around day 300.

### Cross Validation MAE Plot

The MAE (Mean Absolute Error) measures the average magnitude of forecast errors. The model's MAE remains relatively stable at 2,300-2,400 passengers across all forecast horizons, meaning forecasts are typically off by about 2,300-2,400 passengers per day, regardless of whether the model over-predicts or under-predicts.

### Model Forecasting Conclusion

Given the Coverage, RMSE and MAE data, I am pretty confident that the predictions generated using the Prophet model are reasonably accurate if forecasting up to 100 days into the future. The model does become somewhat inaccurate between 100 and 300 days, but not unacceptably. Once we look beyond 300 days into the future, the model begins to lose prediction accuracy.

# Conclusion

As stated in the beginning of this paper, I had four goals in performing this exercise:
...
-   Explore the use of Claude Sonnet in supporting my development of the R code. Although I interacted with Claude as support in reviewing my code and in some cases, correcting and improving it, I did not just ask Claude to write the code for me.

I feel I accomplished these goals.
