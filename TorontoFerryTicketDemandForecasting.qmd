...
```{r}
require(forecast)
sarima_model <- auto.arima(toronto_ferry_data_df$Counts_log,
                           D = 1,
                           max.P = 2, max.Q = 2,
                           seasonal = TRUE,
                           stepwise = FALSE,
...
Box.test(sarima_model$residuals,
         lag = log(length(sarima_model$residuals)),
         type = "Ljung-Box")
```
...
ggplot(toronto_forecast, aes(x = ds)) +
  ...
  scale_fill_manual(values = c("80% Interval" = "lightblue")) +
...
## Cross Validating Phophet Modeling
...
plot_cv_coverage <- ggplot(df_p, aes(x = horizon, y = coverage)) +
  ...
  labs(title = "Coverage Over Forecast Horizon",
       x = "Horizon (days)",
       y = "Coverage",
       subtitle = "Red line = ideal 0.8 coverage") +
  theme_minimal()
