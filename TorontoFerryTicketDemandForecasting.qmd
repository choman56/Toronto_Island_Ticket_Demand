...
The mean ticket redemption count is around `3270` while the median is around 1,120. This substantial difference—with the mean nearly three times the median—indicates a right-skewed distribution driven by high-value outliers that pull the mean upward.
...
```{r}
# Convert possible -inf and inf values in Counts_log to NA
toronto_ferry_data_df$Counts_log[is.infinite(toronto_ferry_data_df$Counts_log)] <- NA

# Now remove the NA rows in Counts_log
toronto_ferry_data_df <- toronto_ferry_data_df[complete.cases(toronto_ferry_data_df), ]
...
toronto_ferry_data_df |>
  dplyr::filter(is.finite(Counts_log)) |>  # Remove non-finite values upfront from log data
...
```{r}
toronto_ferry_data_diff<- diff(as.ts(toronto_ferry_data_df$Counts_log),
                               lag=1, differences = 2)
#toronto_ferry_data_diff_df <- 
#  data.frame(toronto_ferry_data_df$Date[3:length(c())+2])
#toronto_ferry_data_diff_df$Counts_log_diff <- toronto_ferry_data_diff
```
